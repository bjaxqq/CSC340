...
# Linux Version
# Usage: ./rtt_linux.sh <IP_ADDRESS> <REGION_NAME>
...
# Get initial RTT
INITIAL_RTT=$(ping -c 1 $IP | awk -F'=' '/time=/ {print $NF}' | awk '{print $1}')
...
for ((i=1; i<=20; i++)); do
    # Measure SampleRTT
    current_rtt=$(ping -c 1 $IP | awk -F'=' '/time=/ {print $NF}' | awk '{print $1}')
